...
// our logo component that runs a unique animation use gsap
// normally what is suppose to happen is that we are loading our svg and another svg outline that sits on top
// using ref we create reference to our application elements 

const Logo = () => {
    // targets our container
    const bgRef = React.useRef();
    // targets our svg outline path
    const outlineLogoRef = React.useRef();
    // targets our logo image
    const solidLogoRef = React.useRef();

    // basic idea we use gsaps draw svg plugin to trace our svg outline path. 
    // once that animation is nearing completion we load our own log underneath
    // so it creates an effect that looks like the app was tracing the logo image

    React.useEffect(() => {
      // loading the draw svg from gsap
        gsap.registerPlugin(DrawSVGPlugin)
      
        // gsap creating a timeline and begin by calling our container and bringing it into view opacity 0 => 1 1s

        // after we complete this we call our outlineLogo and tell gsap to draw our svg outline for 20seconds
        gsap
          .timeline()
          .to(bgRef.current, {
            duration: 1,
          ...
          // after which we load a inbetween function that as the svg draw is completing 
          // we call our solid image and set its opacity from 0 to 1
          // we delay for 4 seconds and have it last 4 seconds long
        gsap.fromTo(
    ...
    return (
      // container with a large ref to refrence this element
        <div className="logo-container" ref={bgRef}>
          {/* our main logo image with a seperate ref */}
          <img
        ...
        >
          <g
            ...
            fill="none"
          >
            {/* the logo svg path that we create a ref and tie to */}
            <path
              ref={outlineLogoRef}
